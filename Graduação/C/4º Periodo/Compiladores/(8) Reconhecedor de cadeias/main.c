...
main(){
	...
	for(l=0; l<tam; l++){for(c=0; c<tam; c++){matriz[l][0]=0;matriz[l][1]=0;vetor[l]=0;vetorE[l]=0;tipo[l]=0;A[l].string[c]=0;}}
	//Leitura do codigo
	...
	for(qtdL=0, x=1, z=0; fgets(aux, tam, p_cod)!=NULL; qtdL++, x++){
		...
		if(z!=0 && w==0){
			...
			printf("(Estrututa: %i)[Sintatico] Codigo Invalido: Variavel global declarada em local indevido!! [%s]\n\n", tipo[y], A[y].string);
			erro=1;
			for(c=y; c<=qtdL; c++){
	...
	for(z=1; z<=qtdC; z++){
		...
		for(l=0, x=0, y=0; l<qtdL; l++){if(matriz[l][0]==1 && y==0 && z==tipo[l]){x++;y++;}}
		for(l=0, y=0; l<qtdL; l++){if(matriz[l][0]==2 && y==0 && z==tipo[l]){x++;y++;}}
		for(l=0, y=0; l<qtdL; l++){if(matriz[l][0]==3 && y==0 && z==tipo[l]){x++;y++;}}
		for(l=0, y=0; l<qtdL; l++){if(matriz[l][0]==4 && y==0 && z==tipo[l]){x++;y++;}}
		if(x!=4){
			
			printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ausencia de palavra reservada\n\n", z);
			erro=1;
		...
		if(x==0){
			...
			printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ausencia nomeacao do codigo\n\n", z);
			erro=1;
	...
	for(z=1; z<=qtdC; z++){
		...
		for(l=0; l<qtdL; l++){
			...
			if(matriz[l][1]>1 && z==tipo[l]){
				...
				erro=1;
				vetorE[l]=1;
				printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Palavra reservada repetida!! [%s]\n\n", z, A[l].string);
		...
		for(l=0; l<qtdL; l++){
			...
			for(c=0; c<qtdL; c++){
				...
				if(matriz[l][0]==1 && (matriz[c][0]==2 || matriz[c][0]==3 || matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem de palavras reservadas incorreta!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
			...
			if(tipo[l]==z){
				...
				if(matriz[l][0]==7 || matriz[l][0]==10 || matriz[l][0]==13 || matriz[l][0]==16){
					...
					for(c=0; c<10; c++){
						...
						if(A[l].string[0]==num[c]){
							...
							vetorE[l]=1;
							erro=1;
							printf("(%i)[Lexico] Codigo Invalido: Nome de variavel comecando com caractere numeral!! [%s]\n\n", z, A[l].string);
					...
					for(c=l+1; c<qtdL; c++){
						...
						if(x==0 && tipo[l]==tipo[c]){
							...
							if((matriz[l][0]==7 && matriz[c][0]==10) || (matriz[l][0]==10 && matriz[c][0]==7) || (matriz[l][0]==13 && matriz[c][0]==16) || (matriz[l][0]==16 && matriz[c][0]==13) || (matriz[l][0]==matriz[c][0])){
								...
								vetorE[c]=1;
								erro=1;
								printf("(%i)[Semantico] Codigo Invalido: Nome de variavel declarada mais de uma vez!! [%s]\n\n", z, A[l].string);
				...
				if(matriz[l][0]==8 || matriz[l][0]==14){
					...
					if(y==1){
						...
						vetorE[l]=1;
						erro=1;
						printf("(%i)[Lexico] Codigo Invalido: Atribuicoes a variaveis tipo BANANA so recebem numeros!! [%s]\n\n", z, A[l].string);					
				...
				if(matriz[l][0]==11 || matriz[l][0]==17){
					...
					if(y==1){
						...
						vetorE[l]=1;
						erro=1;
						printf("(%i)[Lexico] Codigo Invalido: Atribuicoes a variaveis tipo CAJU so recebem letras!! [%s]\n\n", z, A[l].string);					
		...
		for(l=0; l<qtdL; l++){
			...
			if(tipo[l]==z){
				...
				if(matriz[l][0]==6 || matriz[l][0]==9){
					...
					for(c=0; c<qtdL; c++){
						...
						if(((matriz[c][0]==1 && l<c) || (matriz[c][0]==2 && l>c)) && tipo[l]==tipo[c]){
							...
							vetorE[c]=1;
							erro=1;
							printf("(%i)[Sintatico] Codigo Invalido: Ordem de palavras reservadas incorreta!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);					
				...
				if((matriz[l][0]==8 && matriz[l+1][0]==8) || (matriz[l][0]==11 && matriz[l+1][0]==11)){
					...
					vetorE[l+1]=1;
					erro=1;
					printf("(%i)[Sintatico] Codigo Invalido: Atribuicoes seguidas!! [%s]<->[%s]\n\n", z, A[l].string, A[l+1].string);			
			...
			if(z==tipo[l]){
				...
				if(matriz[l][0]==19){
					...
					if(y==0){
						...
						printf("(%i)[Sintatico] Codigo Invalido: Leitura de dados deve possuir variavel valida!! [%s]\n\n", z, aux);
						vetorE[l]=1;
						erro=1;	
		...
		for(l=0, y=0, z1=0, z3=0; l<qtdL; l++){
			...
			if(z==tipo[l]){
				...
				if(z1==1){
					...
					z1=0;
					printf("(%i)[Sintatico] Codigo Invalido: Deve-se abrir e fechar parantese na escrita de dados!!\n\n", z);
					for(c=z2; c<=k; c++)
						vetorE[c]=1;
					erro=1;					
		...
		for(l=0; l<qtdL; l++){
			...
			if(matriz[l][0]==22 && z==tipo[l]){
				...
				if(z3==0){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Nao se enquadra nas regras de escrita de dados!! [%s]\n\n", z, A[l].string);
					vetorE[l]=1;
					erro=1;	
		...
		for(l=0; l<qtdL; l++){
			...
			if(z==tipo[l] && matriz[l][0]==29){
				...
				if(z3==0){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Nao se enquadra nas regras de escrita do bloco!! [%s]\n\n", z, A[l].string);
					vetorE[l]=1;
					erro=1;	
			...
			if(z==tipo[l]){
				...
				if(matriz[l][1]==1 && x==0){
					...
					if(z1==0){
						...
						printf("(%i)[Sintatico] Codigo Invalido: Essa sequencia nao se enquadra nas regras de escrita do bloco!! [%s %s %s]\n\n", z, A[l].string, A[l+1].string, A[l+2].string);
						vetorE[l]=1;
						erro=1;						
				...
				if(matriz[l][0]==34 && matriz[l+1][0]!=30 && matriz[l+1][0]!=31 && matriz[l+1][0]!=32 && matriz[l+1][0]!=33){
						...
						printf("(%i)[Sintatico] Codigo Invalido: Operador matematico nao se enquadra na sequencia correta!! [%s]\n\n", z, A[l].string);
						vetorE[l]=1;
						erro=1;						
				...
				if(z1==1){
					...
					if(matriz[l][0]==31 || matriz[l][0]==33){
						...
						printf("(%i)[Semantico] Codigo Invalido: Nao e possivel atribuir strings para variaveis tipo banana!! [%s]\n\n", z, A[l].string);
						vetorE[l]=1;
						erro=1;	
				...
				if(z1==2){
					...
					if(matriz[l][0]==30 || matriz[l][0]==32){
						...
						printf("(%i)[Semantico] Codigo Invalido: Nao e possivel atribuir numeros para variaveis tipo caju!! [%s]\n\n", z, A[l].string);
						vetorE[l]=1;
						erro=1;	
			...
			if(z==tipo[l]){
				...
				if(x==y){
					...
					if(z1==0){
						...
						printf("(%i)[Sintatico] Codigo Invalido: Operador matematico nao se enquadra na sequencia correta!!\n\n", z);
						vetorE[l]=1;
						erro=1;					
	...
	for(z=1; z<=qtdC; z++){
		...
		for(l=0; l<qtdL; l++){
			...
			if(z==tipo[l] && matriz[l][0]==36){
				...
				if(z3==0){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Variavel invalida no laco de repeticao!! [%s]\n\n", z, A[l+1].string);
					vetorE[l+1]=1;
					erro=1;						
				...
				if(w==0){matriz[l+2][0]=39;}
				else{
					
					printf("(%i)[Sintatico] Codigo Invalido: Falta da palavra 'de' no laco de repeticao!! [%s]\n\n", z, A[l+2].string);
					vetorE[l+2]=1;
					erro=1;						
				...
				if(w==0){matriz[l+4][0]=39;}
				else{
					
					printf("(%i)[Sintatico] Codigo Invalido: Falta da palavra 'ate' no laco de repeticao!! [%s]\n\n", z, A[l+4].string);
					vetorE[l+4]=1;
					erro=1;						
				...
				if(matriz[l+6][0]!=37){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Falta de frango no laco de repeticao!! [%s]\n\n", z, A[l+6].string);
					vetorE[l+6]=1;
					erro=1;						
		...
		for(l=0, x=0, y=0; l<qtdL; l++){
			...
			if(z==tipo[l]){
				...
				if(y>x){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Frango deve aparecer depois de carne!! [%s]\n\n", z, A[l].string);
					vetorE[l]=1;
					erro=1;							
		...
		if(y!=x){
			...
			printf("(%i)[Sintatico] Codigo Invalido: Deve-se abrir e fechar o laco de repeticao!!\n\n", z);
			erro=1;				
	...
	for(z=1; z<=qtdC; z++){
		...
		for(l=0; l<qtdL; l++){
			...
			if(z==tipo[l] && matriz[l][0]==40){
				...
				w=strncmp(A[l+1].string, "(", tam);
				if(w==0){matriz[l+1][0]=43;}
				else{
					
					printf("(%i)[Sintatico] Codigo Invalido: Falta da palavra '(' na estrutura de condicao!! [%s]\n\n", z, A[l+1].string);
					vetorE[l+1]=1;
					erro=1;						
				...
				if(z3==0){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Variavel invalida na estrutura de condicao!! [%s]\n\n", z, A[l+2].string);
					vetorE[l+2]=1;
					erro=1;						
				...
				w=strncmp(A[l+3].string, "==", tam);
				if(w==0){z3=1;}					
				if(z3==1){matriz[l+3][0]=43;}
				else{
					
					printf("(%i)[Sintatico] Codigo Invalido: Operador logico escrito de modo errado!! [%s]\n\n", z, A[l+3].string);
					vetorE[l+3]=1;
					erro=1;						
				...
				if(matriz[l+6][0]!=41){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Falta de empada na estrutura de condicao!! [%s]\n\n", z, A[l+6].string);
					vetorE[l+6]=1;
					erro=1;						
		...
		for(l=0, x=0, y=0; l<qtdL; l++){
			...
			if(z==tipo[l]){
				...
				if(y>x){
					...
					printf("(%i)[Sintatico] Codigo Invalido: Empada deve aparecer depois de coxinha!! [%s]\n\n", z, A[l].string);
					vetorE[l]=1;
					erro=1;							
		...
		if(y!=x){
			...
			printf("(%i)[Sintatico] Codigo Invalido: Deve-se abrir e fechar a estrutura de condicao!!\n\n", z);
			erro=1;				
		...
		for(l=0; l<qtdL; l++){
			...
			for(c=0; c<qtdL; c++){
				...
				if(matriz[l][0]==18 && (matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem incorreta Leitura de dados inserido em local indevido!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
				...
				if(matriz[l][0]==20 && (matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem incorreta Escrita de dados inserida em local indevido!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
				...
				if(matriz[l][0]==28 && (matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem incorreta Bloco inserido em local indevido!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
				...
				if(matriz[l][0]==36 && (matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem incorreta Laco de repeticao inserido em local indevido!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
				...
				if(matriz[l][0]==40 && (matriz[c][0]==4) && l>c && tipo[l]==tipo[c] && tipo[l]==z){
					...
					vetorE[c]=1;
					erro=1;
					printf("(Estrutura: %i)[Sintatico] Codigo Invalido: Ordem incorreta Estrutura de condicao inserido em local indevido!! [%s]<->[%s]\n\n", z, A[l].string, A[c].string);
	...
	for(z=1; z<=qtdC; z++){
		...
		for(l=0; l<qtdL; l++){
			...
			if(matriz[l][0]==0){
				...
				printf("(%i)[Lexica] Codigo Invalido: Token nao reconhecido!! [%s]\n\n", z, A[l].string);
				matriz[l][0]=999;
				erro=1;
	...
	for(l=0; l<qtdL; l++){
		...
		if(matriz[l][0]!=-1){
			...
			if(matriz[l][0]==21 || matriz[l][0]==22 || matriz[l][0]==23 || matriz[l][0]==24 || matriz[l][0]==25 || matriz[l][0]==26 || matriz[l][0]==27)
				fprintf(log, "\t[dado escrito]\n");
			if(matriz[l][0]==28)
				fprintf(log, "\t[BLOCO]\n");
			if(matriz[l][0]==29 || matriz[l][0]==30 || matriz[l][0]==31 || matriz[l][0]==32 || matriz[l][0]==33 || matriz[l][0]==34 || matriz[l][0]==35)
